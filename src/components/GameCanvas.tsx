...
const GameCanvas: React.FC = () => {
  ...
  // Инициализация мерцающей пыли (улучшенная версия)
  useEffect(() => {
    const updateSize = () => {
      if (containerRef.current) {
        ...
        // Создаём пыль, если её ещё нет
        if (gameRef.current.ambientParticles.length === 0) {
          for (let i = 0; i < 80; i++) {
            // Больше частиц, крупнее, ярче
            gameRef.current.ambientParticles.push({
              x: Math.random() * clientWidth,
              y: Math.random() * GROUND_Y,
              speed: 0.2 + Math.random() * 0.6,
              vy: (Math.random() - 0.5) * 0.3,
              size: 1.5 + Math.random() * 2,   // от 1.5 до 3.5
              baseOpacity: 0.3 + Math.random() * 0.4, // от 0.3 до 0.7
  ...
  // --- Функция обновления положения пыли (вызывается в игровом цикле) ---
  const updateAmbientParticles = (dtFactor: number, speed: number, width: number) => {
    gameRef.current.ambientParticles.forEach(p => {
      // Горизонтальное движение с учётом скорости игры и dtFactor
      p.x -= p.speed * (speed / 5) * dtFactor;
      // Вертикальное покачивание
      p.y += p.vy * dtFactor;

      // Респаун за левой границей
      if (p.x < -20) {
        p.x = width + 20;
        p.y = Math.random() * GROUND_Y;
      }
      // Ограничение по вертикали (чтобы не улетала слишком высоко/низко)
      if (p.y < 0) p.y = GROUND_Y;
      if (p.y > GROUND_Y) p.y = 0;
    });
  };

  // --- Отрисовка монстров (все улучшенные версии) ---
  const drawDragon = (ctx: CanvasRenderingContext2D, x: number, dy: number, width: number, height: number, time: number) => {
    ...
    ctx.beginPath();
    ctx.moveTo(x + width + 30, dy + height - 10);
    ctx.lineTo(x + width + 40, dy + height + 5);
    ctx.lineTo(x + width + 20, dy + height + 5);
    ctx.fill();

    // Крылья
    [
      { ox: 35, sx: 1, color: '#7F1D1D' },
      { ox: 15, sx: -1, color: '#B91C1C' },
    ].forEach(wing => {
      ctx.save();
    ...
    ctx.beginPath();
    ctx.moveTo(x + 20, dy + 5);
    ctx.lineTo(x + 30, dy - 15);
    ctx.lineTo(x + 40, dy + 5);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(x + 15, dy + 22, 10, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = 'black';
    ctx.fillRect(x + 14, dy + 16, 2, 12);
  ...
  const drawOgre = (ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, time: number) => {
    ...
    ctx.beginPath();
    ctx.arc(x + 10, y + 35, 18, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = '#15803d';
    ctx.beginPath();
    ctx.ellipse(x + width / 2, y + height - 25, 20, 15, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fill();
    ctx.beginPath();
    ctx.arc(x + 40, y + 20, 3, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillStyle = '#54300a';
    ctx.fillRect(-2, -40, 2, 55);

    ctx.fillStyle = '#713F12';
    ctx.beginPath();
    ctx.arc(2, -45, 20, 0, Math.PI * 2);
    ctx.fill();
  ...
  const drawMonster = (ctx: CanvasRenderingContext2D, m: Monster) => {
    ...
    if (type === 'DRAGON') {
      ...
      ctx.fill();
      ctx.strokeStyle = '#3F2305';
      ctx.lineWidth = 1;
      for (let i = 5; i < width; i += 10) {
        ctx.beginPath();
        ctx.moveTo(x + i, y + 27);
        ctx.lineTo(x + i, y + height - 4);
        ctx.stroke();
      }
      ctx.fillStyle = 'black';
      ...
      for (let i = 0; i < 6; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(tx, y + 25);
        ctx.lineTo(tx + 3, y + 20);
        ctx.lineTo(tx + 6, y + 25);
        ctx.fill();
      }
      ctx.save();
      ...
      for (let i = 0; i < 7; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(tx, y + 23);
        ctx.lineTo(tx + 3, y + 28);
        ctx.lineTo(tx + 6, y + 23);
        ctx.fill();
      }
      ctx.restore();
    } else if (type === 'BAT') {
      ...
      ctx.beginPath();
      ctx.ellipse(x + width / 2, by + height / 2, 10, 14, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.fillStyle = '#1F2937';
      ctx.beginPath();
      ctx.arc(x + width / 2, by + height / 2 - 10, 8, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = '#EF4444';
      ctx.beginPath();
      ctx.arc(x + width / 2 - 3, by + height / 2 - 10, 2, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(x + width / 2 + 3, by + height / 2 - 10, 2, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.arc(x + width / 2, y + height - 18, 10, 0, Math.PI * 2);
      ctx.fill();
      ...
      for (let i = 0; i < 6; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(x + width / 2, fy + height / 2);
        ctx.lineTo(tx, ty);
        ctx.stroke();
        ctx.fillStyle = 'white';
        ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(
        x + width / 2 + Math.cos(time * 0.004) * 8,
        fy + height / 2 + Math.sin(time * 0.004) * 5,
        8,
        0,
        Math.PI * 2
      );
      ...
      ctx.quadraticCurveTo(
        x + width,
        y + height + hover + Math.sin(time * 0.01) * 10,
        x + width / 2,
        y + height - 10 + hover
      );
      ...
      ctx.beginPath();
      ctx.arc(x + 15, y + 25 + hover, 4, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(x + width - 15, y + 25 + hover, 4, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1.0;
    } else {
  ...
  // --- Отрисовка фона (с улучшенной пылью) ---
  const drawBackground = (ctx: CanvasRenderingContext2D) => {
    const W = ctx.canvas.width;
    const H = ctx.canvas.height;
    const time = Date.now();

    // Самый дальний слой: чёрный фон
    ...
    for (let x = -farOffset; x < W + 400; x += 400) {
      ...
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.arc(tx, ty, 65 + flicker, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = '#F59E0B';
      ctx.beginPath();
      ctx.moveTo(tx - 6, ty + 8);
      ctx.quadraticCurveTo(tx, ty - 15 - flicker, tx + 6, ty + 8);
      ctx.fill();
    }

    // --- Слой мерцающей пыли (только отрисовка, позиции обновлены ранее) ---
    gameRef.current.ambientParticles.forEach(p => {
      const twinkle = Math.sin(time * 0.003 + p.phase);
      const currentOpacity = Math.max(0.1, p.baseOpacity + twinkle * 0.2);

      ctx.fillStyle = '#EAB308';
      ctx.globalAlpha = currentOpacity;

      // Добавляем небольшое свечение (улучшение из второго варианта)
      ctx.shadowBlur = 2;
      ctx.shadowColor = '#EAB308';

      ctx.fillRect(p.x, p.y, p.size, p.size);

      ctx.shadowBlur = 0; // сбрасываем для остальных объектов
    });
    ctx.globalAlpha = 1.0;
    ...
    for (let x = -archOffset; x < W + 500; x += 500) {
      ...
      ctx.fill();
      ctx.strokeStyle = '#1E1A26';
      ctx.lineWidth = 1;
      for (let i = 0; i < 5; i++) {
        ctx.beginPath();
        ctx.moveTo(columnX + 20, 250 + i * 80);
        ctx.lineTo(columnX + columnWidth - 20, 250 + i * 80);
        ctx.stroke();
      }
    }
  ...
  const draw = useCallback(() => {
    ...
    gameRef.current.coins.forEach(coin => {
      ...
      ctx.fillStyle = '#EAB308';
      ctx.beginPath();
      ctx.arc(coin.x + 10, coin.y + 10 + bounce, 8, 0, Math.PI * 2);
      ctx.fill();
  ...
  const handleUpdate = useCallback(
    ({ deltaTime, timestamp }: { deltaTime: number; timestamp: number }) => {
      const dtFactor = deltaTime / 16.67;
      const W = canvasRef.current?.width || 450;
      const currentSpeed = engineRef.current.speed;

      // Всегда обновляем пыль (даже на старте, для красоты)
      updateAmbientParticles(dtFactor, currentSpeed, W);
      ...
      if (gameState === 'PLAYING') {
        ...
        if (selectedClass?.name === 'BARD' && timestamp - lastRegenRef.current > 20000) {
          if (hp < maxHp) {
            heal(1);
            addLog('БАРД: РЕГЕНЕРАЦИЯ (+1 HP)', 'success');
          }
        ...
        // Спавн монстров
        if (
          timestamp - gameRef.current.collisionCooldown > 1800 / (currentSpeed / 5) &&
          Math.random() < 0.04 * dtFactor
        ) {
          const monsterTypes: MonsterType[] = ['SLIME', 'MIMIC', 'BEHOLDER', 'BAT', 'DRAGON', 'OGRE', 'GHOST'];
          const type = monsterTypes[Math.floor(Math.random() * monsterTypes.length)];
          const config =
            ASSET_MANIFEST.MONSTERS[type as keyof typeof ASSET_MANIFEST.MONSTERS] || ASSET_MANIFEST.MONSTERS.SLIME;
        ...
        for (let i = monsters.length - 1; i >= 0; i--) {
          ...
          const monsterConfig =
            ASSET_MANIFEST.MONSTERS[m.type as keyof typeof ASSET_MANIFEST.MONSTERS] || ASSET_MANIFEST.MONSTERS.SLIME;
          if (
              ...
              <div className="flex gap-1.5">
                {Array.from({ length: maxHp }).map((_, i) => (
                  <Heart
                    key={i}
                    size={14}
                    fill={i < hp ? '#ff0000' : 'none'}
                    color={i < hp ? '#ff0000' : '#333'}
                  />
                ))}
              </div>
